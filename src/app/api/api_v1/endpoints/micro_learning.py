...
from src.app.models.db_models import CorpusEmbedding, QtyDocumentInQdrant, MetaDocument, MetaDocumentType
...
async def get_full_journey(lang: str, sdg: int, subject: str):
    ...
    with session_maker() as session:
        ...
        subject_meta_documents: list[MetaDocument] = session.query(
            MetaDocument
        ).join(
            MetaDocumentType, MetaDocumentType.id == MetaDocument.meta_document_type_id
        ).filter(
            MetaDocumentType.meta_document_type == "subject", MetaDocument.title == subject
        ).first()

        if not subject_meta_documents:
            raise ValueError(
    ...
    subject_binary_embedding = subject_meta_documents.embedding
    ...
    for sdg_doc in sdg_meta_documents:
        ...
        try:
            sdg_doc_type = JourneySectionType[sdg_doc.meta_document_type.title.lower()]
        except KeyError:
